    ...
    async e => {
      ...
      try {
        if (editingMessage) {
          ...
          const messageData = {
            content: messageText.trim(), // Text content (can be empty for file-only messages)
            chat: chatId, // The chat ID where the message is being sent
            sender: user._id, // The ID of the sender (current user)
            replyToId: replyingTo?._id, // ID of the message being replied to (if any)
            attachments: files.map(file => ({
              url: file.preview, // URL or preview of the file
              type: file.type, // Type of attachment (image, audio, video, etc.)
              filename: file.name, // Original filename
              size: file.size, // File size in bytes
              mimeType: file.file.type, // MIME type of the file
              duration: file.duration, // Duration for audio/video files (if applicable)
              dimensions: file.dimensions, // Dimensions for images/videos (if applicable)
            })),
          };
